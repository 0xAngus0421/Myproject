...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name in name_data:
        ...
        elif int(rank) < int(name_data[name][year]):
            name_data[name][year] = rank
    else:
        name_data[name] = {year: rank}
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if len(baby_list) == 1:
                year = baby_list[0].strip()

            else:
                rank = baby_list[0].strip()
                boy_name = baby_list[1].strip()
                girl_name = baby_list[2].strip()
